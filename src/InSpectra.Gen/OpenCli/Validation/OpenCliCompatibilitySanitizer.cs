...
internal static class OpenCliCompatibilitySanitizer
{
    ...
    {
        NormalizeAcceptedValues(argument);
        NormalizeStringArrayProperty(argument, "acceptedValues");
        NormalizeObjectArrayProperty(argument, "metadata", static _ => { });
    }

    private static void NormalizeAcceptedValues(JsonObject argument)
    {
        if (argument.ContainsKey("acceptedValues"))
        {
            argument.Remove("allowedValues");
            return;
        }

        if (argument.TryGetPropertyValue("allowedValues", out var allowedValues) && allowedValues is not null)
        {
            argument["acceptedValues"] = allowedValues;
        }
